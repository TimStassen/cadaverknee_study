    ...
    def apply_transformation(self, transformation_file_dir, segmentation_img_dir, fixed_image=None, moving_image=None):
        ...
        # if registratons have been done in the same operation, self.results_list can be used, otherwise the path has to be specified
            for spec_results_path in os.listdir(transformation_file_dir):
                # pdb.set_trace()
                transform_path0 = os.path.join(transformation_file_dir, spec_results_path, 'TransformParameters.0.txt')
                transform_path1 = os.path.join(transformation_file_dir, spec_results_path, 'TransformParameters.1.txt')
                transform_paths = [transform_path0, transform_path1]
                ...
                tr = elastix.TransformixInterface(parameters=transform_paths, transformix_path=self.transformix_path)
